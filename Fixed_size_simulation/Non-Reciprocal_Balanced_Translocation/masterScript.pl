...
`perl ../TranslocationScripts/generateFrameWork.Intra.pl chrA.Intra.conf`;
`perl ../TranslocationScripts/generateFrameWork.Intra.pl chrB.Intra.conf`;
`perl ../TranslocationScripts/swapping.transFrameWork.pl chrAB.Inter.conf`;
`perl ../TranslocationScripts/transformPreSimToSimMatrix.pl $chrA_name.modified.index $outFileName.chrA.Intra.$chrA_name.PreSimulation.matrix $chrA_count_treatment $chrB_name.modified.index $outFileName.chrB.Intra.$chrB_name.PreSimulation.matrix $chrB_count_treatment $outFileName.chrA.Intra.$chrA_name.Simulation.matrix $outFileName.chrB.Intra.$chrB_name.Simulation.matrix`;
`perl ../TranslocationScripts/generateInterMatrix.pl $outFileName.chrAB.Inter.$chrA_name-$chrB_name.Simulation.matrix`;
`perl ../TranslocationScripts/generateIntraMatrix.pl $outFileName.chrA.Intra.$chrA_name.Simulation.matrix > $chrA_name.Intra.sparse.matrix`;
`perl ../TranslocationScripts/generateIntraMatrix.pl $outFileName.chrB.Intra.$chrB_name.Simulation.matrix > $chrB_name.Intra.sparse.matrix`;
`rm *.Intra-Inter.sparse.matrix`;
`cat *.sparse.matrix > $chrA_name.$chrB_name.Intra-Inter.sparse.matrix`;
`perl ../TranslocationScripts/createSparseToMatrix.pl $outFileName.New.bed $chrA_name.$chrB_name.Intra-Inter.sparse.matrix $outFileName.$chrA_name.$chrB_name.Figure.axis $outFileName.$chrA_name.$chrB_name.Figure.matrix`;
`Rscript ../TranslocationScripts/figure.translocation.r $outFileName.$chrA_name.$chrB_name.Figure.matrix $outFileName.$chrA_name.$chrB_name.Figure.axis $outFileName.$chrA_name.$chrB_name.Figure.png`;
